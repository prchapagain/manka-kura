...
const VoiceRecorderButton: React.FC<VoiceRecorderButtonProps> = ({ onRecordingComplete, onRecordingError }) => {
    ...
    <button
      ...
      className={`flex items-center justify-center space-x-2 px-4 py-2 border rounded-lg transition-colors
                  ${isRecording ? 'bg-red-500 hover:bg-red-600 text-white border-red-500' 
                               : 'bg-transparent border-primary text-primary hover:bg-primary-light hover:text-primary-dark dark:hover:bg-primary-dark dark:hover:text-primary-light'}`}
    >
      <MicrophoneIcon className="w-5 h-5" />
      <span>
        {isRecording ? `${translate('recording')} (${countdown}s)` : translate('recordVoice')}
      </span>
    </button>
  );
};
